...
export const createAbout = (aboutData) => (dispatch) => {
  ...
  fetch('http://localhost:3005/about/create', {
    method: 'POST',
...
export const getAllAbouts = () => (dispatch) => {
  ...
  fetch('http://localhost:3005/about/abouts')
    .then((response) => response.json())
    .then((data) => dispatch({
...
export const getAboutById = (id) => async (dispatch) => {
    ...
    try {
      const response = await fetch(`http://localhost:3005/about/abouts/${id}`);
...
export const updateAbout = (id, aboutData) => (dispatch) => {
  ...
  fetch(`http://localhost:3005/about/update/${id}`, {
      method: 'PUT',
      body: formData,
  })
  .then(response => {
...
export const deleteAbout = (id) => (dispatch) => {
    ...
    fetch(`http://localhost:3005/about/delete/${id}`, {
        method: 'DELETE',
    })
    .then(response => {
        if (!response.ok) {
...
export const countAbouts = () => (dispatch) => {
  ...
  fetch('http://localhost:3005/about/count')
    .then((response) => response.json())
    .then((data) => dispatch({
